...
class Appr(object):
    ...
    def __init__(self, model, nepochs=100, sbatch=64, lr=0.05, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=10000,
                 lamb=0.75, smax=400, use_cuda=True, args=None):
        self.model = model
    ...
    def train(self, t, xtrain, ytrain, xvalid, yvalid):
        ...
        with torch.no_grad():
            task = torch.autograd.Variable(t_tensor)
        ...
        if t == 0:
            ...
            for i in range(len(self.mask_pre)):
                freeze = (torch.cuda.FloatTensor(self.mask_pre[i].shape).uniform_() > 0.1).float()
                self.mask_pre[i] = torch.max(self.mask_pre[i], freeze)
        else:
            for i in range(len(self.mask_pre)):
                self.mask_pre[i] = torch.max(self.mask_pre[i], mask[i])

        for i in range(len(self.mask_pre)):
            print(self.mask_pre[i].sum()/self.mask_pre[i].nelement())
    ...
    def eval(self, t, x, y):
        ...
        for i in range(0, len(r), self.sbatch):
            ...
            # Log
            t_loss = loss
            t_total_acc = hits.sum()
            t_reg = reg
            if self.use_cuda:
                t_loss = t_loss.data.cpu().numpy()
                t_total_acc = t_total_acc.data.cpu().numpy()
                t_reg = t_reg.data.cpu().numpy()

            total_loss += t_loss.item() * len(b)
            total_acc += t_total_acc.item()
            total_num += len(b)
            total_reg += t_reg.item() * len(b)
