...
class Net(torch.nn.Module):
    ...
    def __init__(self, inputsize, taskcla):
        ...
        self.c1_xda = XdA([1, 64, 29, 29])
        self.c2_xda = XdA([1, 128, 12, 12])
        self.c3_xda = XdA([1, 256, 5, 5])
        self.fc1_xda = XdA([1, 2048])
        self.fc2_xda = XdA([1, 2048])
        return

    def forward(self, x):
        h = self.c1_xda(self.conv1(x))
        h = self.maxpool(self.drop1(h))
        h = self.c2_xda(self.conv2(h))
        h = self.maxpool(self.drop1(h))
        h = self.c3_xda(self.conv3(h))
        h = self.maxpool(self.drop2(h))
        h = h.view(x.size(0), -1)
        h = self.fc1_xda(self.fc1(h))
        h = self.drop2(h)
        h = self.fc2_xda(self.fc2(h))
        h = self.drop2(h)
        y = []
